...
public:
    ...
    double Eval(const Eigen::Vector3d &_x) {
        ...
        double firstPart = pow(m_center.x() - _x.x(), 2) + // x component
                           pow(m_center.y() - _x.y(), 2) + // y component
                           pow(m_center.z() - _x.z(), 2) + // z component
                           pow(m_radius, 2) - // ring radius
                           pow(m_a, 2); // small radius
        double secondPart = pow(m_radius, 2) *
                            (
                                    pow(m_center.x() - _x.x(), 2) +
                                    pow(m_center.y() - _x.y(), 2)
                            );

        return pow(firstPart, 2) - 4 * secondPart;
    }
